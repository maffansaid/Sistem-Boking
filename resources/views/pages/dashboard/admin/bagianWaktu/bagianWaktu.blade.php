        ...
        <div class="m-5 lg:m-20">
                ...
                <div class="flex gap-4 md:justify-center">
                    <form class="flex w-full flex-col">
                        ...
                        <input name="tanggal" type="date" id="dateInput"
                            ...
                            placeholder="" value="{{ $tanggal->format('Y-m-d') }}" onchange="this.form.submit()" />
                    </form>
            ...
            <div class="mt-4">
                <form id="form" action="" method="POST"
                    class="grid grid-cols-3 gap-2 p-4 lg:grid-cols-5 lg:gap-4">
                    @csrf
                    ...
                    @foreach ($waktu as $item)
                        ...
                        <div class="flex items-center justify-center">
                            <input type="checkbox" id="{{ $item->id }}" name="waktu[]" class="peer hidden"
                                value="{{ $item->id }}" @checked($item->terhapus->count()) @disabled($disabled) />
                            <label for="{{ $item->id }}"
                                class="{{ $disabled ? 'bg-yellow-500 cursor-pointer transition transform duration-300 hover:scale-110' : 'bg-[#81C125] hover:bg-[#81C125] peer-checked:bg-gray-300 peer-focus:bg-gray-300 cursor-pointer transition transform duration-300 hover:scale-110' }} rounded-full border-none p-4 text-center text-white cursor-pointer transition transform duration-300 hover:scale-110">{{ Str::substr($item->waktu, 0, 5) }}</label>
                        </div>
                    @endforeach
                </form>
                <div class="flex justify-end">
                    <button type="button" onclick="document.getElementById('form').submit()"
                        class="mt-4 block rounded-lg bg-[#F8C104] px-5 py-2.5 text-sm font-bold text-black hover:bg-[#B58D05] focus:outline-none focus:ring-4 focus:ring-blue-300 font-bold transition transform duration-300 hover:scale-110">Kirim
                        Pilihan</button>
                </div>
                <div
                    class="max-w-md mx-auto my-4 rounded-lg bg-gradient-to-r from-white via-white to-white p-6 shadow-lg">
                    <h2 class="text-lg font-bold text-gray-800 mb-4 text-center">Informasi</h2>
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-gray-200">
                                <th class="p-2 border border-black text-gray-800">Warna</th>
                                <th class="p-2 border border-black text-gray-800">Keterangan</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="border border-black">
                                <td class="p-2 flex items-center">
                                    <div class="h-4 w-4 rounded-full bg-[#81C125] hover:bg-[#81C125] mr-3"></div>
                                    Hijau
                                </td>
                                <td class="p-2 border border-black">Aktif</td>
                            </tr>
                            <tr class="border border-black">
                                <td class="p-2 flex items-center">
                                ...
                                <td class="p-2 border border-black">Nonaktif</td>
                            </tr>
                            <tr class="border border-black">
                                <td class="p-2 flex items-center">
                                ...
                                <td class="p-2 border border-black">Terbooking</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
